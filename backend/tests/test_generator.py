...
async def clean_db(engine):
    async with engine.begin() as conn:
        await conn.execute(
            text("TRUNCATE conversations, components_log, generation_results, requests RESTART IDENTITY CASCADE")
        )
...
async def test_landing_with_slider_generates_both(db_session: AsyncSession):
    """Landing + slider: HTML must contain both hero section and image-slider."""
    get_llm_client.cache_clear()
    os.environ["LLM_MODE"] = "mock"

    result = await generate_interface("лендинг со слайдером", db_session)

    assert result["html"]
    assert 'class="hero"' in result["html"]
    assert 'class="image-slider"' in result["html"]
